...
BASE_DATABASES_DIR = "YOUR PATH/database"
...
def load_spider_train_set():
    df1 = pd.read_json("YOUR PATH/train_others.json")
    df2 = pd.read_json("YOUR PATH/train_spider.json")
...
def load_spider_dev_set():
    df = pd.read_json("YOUR PATH/dev.json")
    return df
...
if __name__ == "__main__":
    ...
    df.to_csv("full_finetuning_dataset.csv", index=False)
    df = pd.DataFrame(filtered_finetuning_dataset)
    df.to_csv("filtered_finetuning_dataset.csv", index=False)
    ...
    df.to_csv("validation_dataset_formatted.csv", index=False)
    df = pd.DataFrame(filtered_validation_dataset)
    df.to_csv("filtered_validation_dataset.csv", index=False)
